CREATE TABLE IF NOT EXISTS cloud_usage_reservations (
    ...
    reserved_network_bytes BIGINT NOT NULL CHECK (reserved_network_bytes >= 0),
    actual_cost_micros BIGINT CHECK (actual_cost_micros IS NULL OR actual_cost_micros >= 0),
    actual_network_bytes BIGINT CHECK (actual_network_bytes IS NULL OR actual_network_bytes >= 0),
...
CREATE TABLE IF NOT EXISTS cloud_usage_locks (
    ...
    marker BIGINT NOT NULL DEFAULT 1 CHECK (marker = 1)
);
